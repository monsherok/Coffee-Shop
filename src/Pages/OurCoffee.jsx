...
import OurCoffeeCard from "../components/OurCoffeeCard";

function OurCoffee({ arr }) {
  const categories = ["All", "Kenya", "Brasil", "Columbia"];
  ...
  const searchEpm = (items, term) => {
    if (term.length === 0) {
      return items;
    }

    return items.filter((item) => {
      return item.descr.toLowerCase().indexOf(term.toLowerCase()) > -1;
    });
  };

  const onFiltered = (i) => {
    setActiveButton(i);
    setArray(filterPost(arr, categories[i]));
  };

  return (
    <>
      <div className="header header__ourcoffee">
        <HeaderMenu />
        <h1 className="header__content-title">Our Coffee</h1>
      </div>
      <div className="ourcoffee__container">
        <div className="about__beans">
          <div className="about__beans-image">
            <img src="./images/img-for-best.jpg" alt="" />
          </div>
          <div className="about">
            <h3 className="about__title">About our beans</h3>
        ...
        <div className="ourcoffee__decoration"></div>
        <div className="ourcoffee__items">
          <div className="ourcoffee__items-filters">
            <div className="ourcoffee__items-search">
              <label htmlFor="inputSearch">Looking for</label>
              <input
                onInput={(e) => setArray(searchEpm(arr, e.target.value))}
                id="inputSearch"
                type="text"
                placeholder="start typing here..."
              />
            </div>
            <div className="ourcoffee__items-buttons">
              <p className="ourcoffee__items-buttons-text">Or filter</p>
              {categories.map((item, i) => (
                <button
                  onClick={() => onFiltered(i)}
                  key={i}
                  className={
                    activeButton === i
                      ? "ourcoffee__items-button active"
                      : "ourcoffee__items-button"
                  }
                >
                  {item}
                </button>
          ...
          <div className="ourcoffee__items-inner">
            {array.map((item, index) => (
                  <OurCoffeeCard key={index} {...item} />
                ))}
          </div>
        </div>
      </div>
